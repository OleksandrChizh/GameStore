...
using Resources;
using IEmailService = GameStore.WebUI.EmailService.IEmailService;
using ISmsService = GameStore.WebUI.SmsService.ISmsService;
...
namespace GameStore.WebUI.Controllers
{
    ...
    public class BasketController : BaseController
    {
        ...
        [HttpPost]
        public async Task<ActionResult> MakeRequestToBank(VisaPaymentViewModel model)
        {
            ...
            // throws exception, when basket doesn't exists
            GetBasket();

            if (!await _paymentService.IsUserExistsAsync(model.CardNumber, model.CartHoldersName))
            {
            ...
            {
                await _smsService.SendAsync(model.PhoneNumber, sendingMessage);
            }

            if (model.ConfirmationType == ConfirmationType.UsingEmail)
            {
                await _emailService.SendAsync(model.Email, "Code", sendingMessage);
            }
        ...
        public async Task<ActionResult> ConfirmPayment(ConfirmPaymentViewModel model)
        {
            ...
            PaymentStatus result = await PayOrderUsingPaymentService(data, basket, amountOfPayment);
        ...
        private async Task<PaymentStatus> PayOrderUsingPaymentService(VisaPaymentViewModel data, List<Purchase> basket, decimal amountOfPayment)
        {
            ...
            PaymentStatus result = await _paymentService.PayUsingVisaAsync(
                data.CardNumber,
